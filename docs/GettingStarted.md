...
-- Next, we create our InputMap. This stores our mappings from inputs to actions. This object can be used for rebinding.
...
RunService:BindToRenderStep("Spark", Enum.RenderPriority.Input, function()
    -- First we update our Actions object given the InputState and the associated InputMap.
    actions:update(inputState, inputMap)

    -- InputState:clear needs to be called after updating all Actions objects. It resets the values of the mouse wheel
    -- and mouse movement so they don't persist across frames.
    inputState:clear()
end)
```
